...
export class Card extends Component<IProduct> {
	...
	constructor(container: HTMLElement, actions?: ICardActions) {
		...
		if (actions?.onClick) {
            // Если есть кнопка (для превью или корзины), вешаем на нее
            if (this._button) {
                this._button.addEventListener('click', actions.onClick);
            } else {
                 // Иначе (для каталога) вешаем на всю карточку
    ...
    set category(value: string) {
        ...
        // Удаляем старые классы категорий, если они есть
        this._category?.classList.forEach(className => {
            if (className.startsWith('card__category_') && className !== 'card__category') {
    ...
    set buttonText(value: string) { this.setText(this._button, value); }

    // Для номера в корзине
    set index(value: number) { this.setText(this._index, String(value)); }

    // Метод для управления состоянием кнопки в карточке превью
    setInBasket(inBasket: boolean, price: number | null): void {
        if (this._button) {
            if (price === null) { // Если товар бесценен, кнопка неактивна
